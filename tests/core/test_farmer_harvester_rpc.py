...
from mint.plotting.util import stream_plot_info_ph, stream_plot_info_pk, PlotRefreshResult, PlotRefreshEvents
from mint.plotting.manager import PlotManager
from mint.protocols import farmer_protocol
from mint.rpc.farmer_rpc_api import FarmerRpcApi
from mint.rpc.farmer_rpc_client import FarmerRpcClient
from mint.rpc.harvester_rpc_api import HarvesterRpcApi
from mint.rpc.harvester_rpc_client import HarvesterRpcClient
from mint.rpc.rpc_server import start_rpc_server
...
from mint.wallet.derive_keys import master_sk_to_wallet_sk, master_sk_to_pooling_authentication_sk
...
class TestRpc:
    ...
    async def test1(self, simulation):
        ...
        try:
            ...
            await client.set_reward_targets(encode_puzzle_hash(new_ph, "xkm"), encode_puzzle_hash(new_ph_2, "xkm"))
            ...
            await client.set_reward_targets(None, encode_puzzle_hash(new_ph_3, "xkm"))
            ...
            assert config["farmer"]["xkm_target_address"] == encode_puzzle_hash(new_ph, "xkm")
            assert config["pool"]["xkm_target_address"] == encode_puzzle_hash(new_ph_3, "xkm")

            new_ph_3_encoded = encode_puzzle_hash(new_ph_3, "xkm")
