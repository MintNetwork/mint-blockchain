...
from mint.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from mint.protocols import wallet_protocol
from mint.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from mint.server.node_discovery import WalletPeers
...
from mint.util.ints import uint32, uint128
from mint.util.keychain import Keychain
from mint.util.lru_cache import LRUCache
from mint.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from mint.wallet.settings.settings_objects import BackupInitialized
...
from mint.wallet.wallet_blockchain import ReceiveBlockResult
from mint.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'mint keys' program.")
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSMintConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSMintConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSMintConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSMintConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSMintConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSMintConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
