...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if "xkm_target_address" not in config["farmer"]:
        print(f"Setting the xkm destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "xkm_target_address" not in config["pool"]:
        print(f"Setting the xkm destination address for coinbase reward to {all_targets[0]}")
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if mint_init(root_path, fix_ssl_permissions=fix_ssl_permissions) == 0 and root_path.exists():
                return init(create_certs, root_path, fix_ssl_permissions)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return mint_init(root_path, fix_ssl_permissions=fix_ssl_permissions)
...
def mint_init(root_path: Path, *, should_check_keys: bool = True, fix_ssl_permissions: bool = False):
    """
