...
from mint.cmds.configure import configure_cmd
from mint.cmds.farm import farm_cmd
from mint.cmds.init import init_cmd
from mint.cmds.keys import keys_cmd
from mint.cmds.netspace import netspace_cmd
from mint.cmds.passphrase import passphrase_cmd
from mint.cmds.plots import plots_cmd
from mint.cmds.show import show_cmd
from mint.cmds.start import start_cmd
from mint.cmds.stop import stop_cmd
from mint.cmds.wallet import wallet_cmd
from mint.cmds.plotters import plotters_cmd
from mint.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from mint.util.keychain import set_keys_root_path, supports_keyring_passphrase
from mint.util.ssl_check import check_ssl
...
@click.group(
    help=f"\n  Manage mint blockchain infrastructure ({__version__})\n",
    epilog="Try 'mint start node', 'mint netspace -d 192', or 'mint show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        from mint.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
if not supports_keyring_passphrase():
    from mint.cmds.passphrase_funcs import remove_passphrase_options_from_cmd

    # TODO: Remove once keyring passphrase management is rolled out to all platforms
    remove_passphrase_options_from_cmd(cli)


@cli.command("version", short_help="Show mint version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs mint daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by mint start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from mint.daemon.server import async_run_daemon
