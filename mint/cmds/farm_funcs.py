...
from mint.cmds.units import units
...
from mint.util.misc import format_bytes
from mint.util.misc import format_minutes
from mint.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total mint farmed: {amounts['farmed_amount'] / units['mint']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['mint']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['mint']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'mint start wallet' and 'mint wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'mint wallet show'")
    else:
        print("Note: log into your key using 'mint wallet show' to see rewards for each key")
