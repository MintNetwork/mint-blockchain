...
DEFAULT_USER = f"user-mint-{CURRENT_KEY_VERSION}"  # e.g. user-mint-1.8
DEFAULT_SERVICE = f"mint-{DEFAULT_USER}"  # e.g. mint-user-mint-1.8
...
def supports_keyring_passphrase() -> bool:
    # Support can be disabled by setting MINT_PASSPHRASE_SUPPORT to 0/false
    return os.environ.get("MINT_PASSPHRASE_SUPPORT", "true").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from mint.cmds.passphrase_funcs import prompt_for_passphrase
