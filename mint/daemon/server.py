...
from mint.cmds.init_funcs import check_keys, mint_init
from mint.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from mint.daemon.keychain_server import KeychainServer, keychain_commands
from mint.daemon.windows_signal import kill
from mint.plotters.plotters import get_available_plotters
from mint.plotting.util import add_plot_directory
from mint.server.server import ssl_context_for_root, ssl_context_for_server
...
from mint.util.json_util import dict_to_json_str
from mint.util.keychain import (
    Keychain,
...
from mint.util.service_groups import validate_service
from mint.util.setproctitle import setproctitle
from mint.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Mint.")
    quit()
...
if getattr(sys, "frozen", False):
    name_map = {
        "mint": "mint",
        "mint_wallet": "start_wallet",
        "mint_full_node": "start_full_node",
        "mint_harvester": "start_harvester",
        "mint_farmer": "start_farmer",
        "mint_introducer": "start_introducer",
        "mint_timelord": "start_timelord",
        "mint_timelord_launcher": "timelord_launcher",
        "mint_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["mint", "plotters", plotter]
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("mint daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered MINT_ROOT
    os.environ["MINT_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with MINT_ROOT: {os.environ['MINT_ROOT']}")
    ...
    if service_command == "mint_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in mint_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    mint_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("mint_daemon")
