...
from mint.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from mint.plotters.chiapos import get_chiapos_install_info, plot_mint
from mint.plotters.madmax import get_madmax_install_info, plot_madmax
from mint.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in MINT_ROOT.
    mint_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within MINT_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, mint_plotter, "chiapos", "Chiapos Plotter")
    ...
    if args.plotter == "chiapos":
        plot_mint(args, mint_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, mint_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, mint_root_path, root_path)
